...
#define FONT_PATH "../../GameClient/resources/courbd.ttf"
...
struct Sala
{
	...
	Sala(std::string _texto, int _origenX, int _origenY, int _longitudX, int _longitudY, sf::Color _color) :
		texto(_texto), origen(sf::Vector2f(_origenX, _origenY)), longitud(sf::Vector2f(_longitudX, _longitudY)), color(_color) {}
